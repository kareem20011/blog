        ...
        <div class="row">
            ...
            <div class="col-lg-4">
                <div class="d-flex align-items-center justify-content-between bg-light py-2 px-4 mb-3">
                    <h3 class="m-0">{{__('words.category')}}</h3>
                    <a class="text-secondary font-weight-medium text-decoration-none" href="#">{{__('words.viewAll')}}</a>
                </div>
                @foreach($categories as $category)
                <div class="position-relative overflow-hidden mb-3" style="height: 80px;">
                    <img class="img-fluid w-100 h-100" src="{{asset($category->image)}}" style="object-fit: cover;">
                    <a href="{{route('category', $category->id)}}" class="overlay align-items-center justify-content-center h4 text-white text-decoration-none">
                        {{$category->title}}
                    </a>
                </div>
                @endforeach
